      ...
      <div className="flex items-center justify-between pt-space-xs mt-1">
        <div>
          ...
          <p className="font-body-sm text-body-sm text-text-muted">
            CDSCO &amp; NMC compliant generic conversion vault
          </p>
        </div>
        <button
      ...
      <div className="p-space-base rounded-xl bg-status-info-bg border border-status-info-border flex items-start gap-space-sm shadow-sm">
        ...
        <div className="flex flex-col min-w-0">
          <span className="font-label-md text-label-md font-bold text-text-primary">
            AI Salt Molecule Recognition Active
          </span>
          <p className="font-body-sm text-body-sm text-text-secondary mt-0.5">
            Uploaded doctor prescriptions are automatically scanned for brand names (like Dolo, Augmentin, Lipitor) and mapped directly to approved CDSCO generic formulations.
          </p>
        </div>
      </div>
        ...
        {prescriptions.map((rx) => (
          ...
          >
            <div className="flex items-start justify-between border-b border-border-subtle/60 pb-space-xs">
              ...
              <span className="font-code-tabular text-[11px] text-text-muted bg-surface-subtle px-2 py-0.5 rounded font-semibold">
                {rx.id}
              </span>
            </div>
            ...
            <div className="flex flex-col gap-space-xs pt-1">
              <span className="font-label-sm uppercase tracking-wide text-text-muted font-bold">
                Prescribed vs Generic Molecule Match
              </span>
              <div className="flex flex-col gap-2">
                {rx.medicines.map((med, idx) => (
                  <div
                    key={idx}
                    className="flex items-center justify-between p-2 rounded-lg bg-surface-subtle/80 border border-border-subtle"
                  >
                    <div className="flex flex-col">
                      <span className="font-body-sm text-text-muted line-through">
                        {med.prescribed}
                      </span>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="px-2.5 py-1 bg-surface-card text-primary rounded font-label-sm font-bold border border-border-subtle hover:bg-surface-container active:scale-95"
                      >
                        Compare
                      </button>
                    </div>
